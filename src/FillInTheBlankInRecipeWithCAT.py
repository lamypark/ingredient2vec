...
class CBOWModule(nn.Module):
    ...
    def forward(self, x, emb_mask, x_cnt, step):
        ...
        x = x[idx].view(-1, self.input_size)
...
def print_score(batches, step):
    ...
    for batch in batches:
        ...
        for i in xrange(len(target)):
            if target[i] in batch_o[i][:5]:
                print id2ingr[target[i]], [id2ingr[o] for o in batch_o[i][:5]]
...
###############################################################################################
cbow_model = CBOWModule(feat_dim*(max_ingr_cnt-1), len(id2ingr)).cuda()
